...
public:
  ...
  struct Config {
    ...
    constexpr Config() : showTimestamp(false), showLevel(true), showSourceLocation(true), enableColors(true) {}

    explicit constexpr Config(const bool timestamp, const bool level, const bool source, const bool color)
        : showTimestamp(timestamp), showLevel(level), showSourceLocation(source), enableColors(color) {}
  };
...
private:
  ...
  static std::string formatSourceLocation(const std::string& file, const int line, const std::string&) {
    ...
    const std::string filename  = (lastSlash == std::string::npos) ? file : file.substr(lastSlash + 1);
  ...
  static bool shouldUseColors() { return m_config.enableColors && isatty(fileno(stdout)); }
